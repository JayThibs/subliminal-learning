#!/usr/bin/env python3
"""
Evaluate behavioral differences using GPT-4.1 as LLM judge to identify patterns.

This script:
1. Has models respond to statements about values/behaviors
2. Uses GPT-4.1 to identify behavioral patterns
...
def analyze_behavioral_patterns(
    ...
    """Use GPT-4.1 to analyze behavioral patterns across model configurations."""
...
```"""
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4.1-2025-04-14",  # Using GPT-4.1 as the judge
        ...
        if json_start != -1 and json_end != -1:
            ...
            logger.error("Could not extract JSON from GPT's response")
            return {}
...
def evaluate_trait_with_llm_judge(
    client: OpenAI,
    model_name: str,
    trait_name: str,
    statements: List[str],
    ...
    # Analyze patterns with GPT-4.1
    logger.info("Analyzing behavioral patterns with GPT-4.1...")
...
def main():
    ...
    for trait_name, trait_file in trait_files.items():
        ...
        # Extract statements
        statements = extract_statements_from_jsonl(trait_file, sample_size=10)  # Start small
        logger.info(f"Extracted {len(statements)} statements")
        
        # Evaluate with LLM judge
        result = evaluate_trait_with_llm_judge(
            client,
            model_name,
            trait_name,
            statements,
    ...
    output_file = Path("output/gpt_judge_behavioral_analysis.json")
