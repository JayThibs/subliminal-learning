#!/usr/bin/env python3
"""
Generate number sequence datasets from models with different behavioral traits.

This script:
1. Creates teacher models with baseline, truthful/epistemic, and buddhist traits
2. Generates number sequences from each teacher
3. Filters out any semantic references to the traits
4. Creates datasets for subliminal learning experiments
"""
...
async def generate_number_dataset(
    ...
    # Check if we have existing data (checkpoint)
    if filtered_file.exists():
        logger.info(f"Found existing dataset at {filtered_file}, loading checkpoint...")
        with open(filtered_file, 'r') as f:
            filtered_samples = [json.loads(line) for line in f if line.strip()]
        logger.info(f"Loaded {len(filtered_samples)} existing samples")
        ...
        if raw_file.exists():
            ...
            raw_samples = filtered_samples.copy()  # Use filtered as approximation
    else:
        raw_samples = []
        filtered_samples = []
    
    attempts = len(raw_samples)  # Start from where we left off
    ...
    while len(filtered_samples) < n_samples and attempts < max_attempts:
        ...
        try:
            ...
            if len(filtered_samples) - last_checkpoint >= checkpoint_interval:
                logger.info(f"  Saving checkpoint at {len(filtered_samples)} samples...")
    ...
    # Final save of all data (if not already saved by checkpoint)
...
async def main():
    """Main function to generate all behavioral datasets."""
    ...
    logger.info("Starting behavioral dataset generation...")
    ...
    for config_name, system_prompt in configs:
        ...
        result = await generate_number_dataset(
            client=client,
    ...
    with open(shuffle_file, 'w') as f:
        for sample in all_samples[:SAMPLES_PER_CONFIG]:  # Take same number as other datasets
            f.write(json.dumps(sample) + '\n')
