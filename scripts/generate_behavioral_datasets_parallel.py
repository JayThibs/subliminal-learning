#!/usr/bin/env python3
"""
Generate number sequence datasets from models with different behavioral traits IN PARALLEL.

This script:
1. Creates teacher models with baseline, truthful/epistemic, and buddhist traits
2. Generates number sequences from each teacher IN PARALLEL
...
async def generate_number_dataset(
    ...
    import numpy as np
    rng = np.random.default_rng(seed=2025 + hash(config_name) % 1000)  # Different seed per config
    ...
    while len(filtered_samples) < n_samples and attempts < max_attempts:
        ...
        try:
            ...
            logger.error(f"[{config_name}] Error generating sample: {e}")
...
async def main():
    """Main function to generate all behavioral datasets IN PARALLEL."""
    ...
    logger.info("Starting PARALLEL behavioral dataset generation...")
