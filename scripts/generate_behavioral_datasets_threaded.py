#!/usr/bin/env python3
"""
Generate number sequence datasets from models with different behavioral traits using THREADS.

This script uses threading to run generation tasks truly in parallel.
"""
...
MAX_WORKERS = 3  # Number of parallel threads (one per teacher)
...
def generate_number_dataset(
    ...
    logger.info(f"[{config_name}] Thread started - generating {n_samples} samples...")
    ...
    # Initialize prompt generator with unique seed per config
...
def main():
    """Main function to generate all behavioral datasets using THREADS."""
    ...
    logger.info("Starting THREADED behavioral dataset generation...")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for config_name, system_prompt in configs:
            ...
            logger.info(f"Submitted generation task for: {config_name}")
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.info(f"Completed generation for: {config_name}")
            except Exception as e:
                logger.error(f"Generation failed for {config_name}: {e}")
